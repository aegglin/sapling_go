...
func (mapTileHandler *MapTileHandler) DrawAll(screen *ebiten.Image, beetle Beetle) {

	op := &ebiten.DrawImageOptions{}
	op.GeoM.Scale(imageScale, imageScale)

	var mapTileNumber int
	...
	for row := range numWorldRows {
		for col := range numWorldColumns {
			...
			cameraX := worldX - beetle.x + beetle.cameraViewX
			cameraY := worldY - beetle.y + beetle.cameraViewY

			if worldX+tileSize > beetle.x-beetle.cameraViewX &&
				worldX-tileSize < beetle.x+beetle.cameraViewX &&
				worldY+tileSize > beetle.y-beetle.cameraViewY &&
				worldY-tileSize < beetle.y+beetle.cameraViewY {
				// GeoM.Translate is relative to the current position

				op.GeoM.Translate(float64(cameraX), float64(cameraY))

				// if row == 0 && col == 0 {
				// 	op.GeoM.Translate(0, 0)
				// } else if col == numWorldColumns-1 {
				// 	op.GeoM.Translate(-(numWorldRows-1)*tileSize, tileSize)
				// } else {
				// 	op.GeoM.Translate(tileSize, 0)
				// }

				mapTileNumber = mapTileHandler.mapTileNumbers[row][col]
				image = mapTileHandler.mapTiles[mapTileNumber].Image
